...
// Helper: renderCell determines if a cell should render a village
const renderCell = (x: number, y: number, village: Village | undefined): boolean => {
    return !!village
}
...
onMounted(() => {
    ...
    // Scroll map to center cell after villages are fetched
    const center = Math.floor(gridSize / 2)
    smoothScrollToCell(center, center)
})
...
<template>
    <div class="flex h-full overflow-hidden">
        ...
        <main
            ref="mapContainer"
            class="flex-1 relative overflow-auto bg-green-200"
            ...
            >
                ...
                >
                    <template v-if="(() => {
                        ...
                        const y = getCellY(index)
                        const village = getVillageAt(x, y)

                        return renderCell(x, y, village)
                      })()">
                        <div
                            ...
                            :class="[
                                        'border border-gray-300 flex items-center justify-center p-[1px] cursor-pointer text-xs',
                                selectedCell.x === getCellX(index) && selectedCell.y === getCellY(index) ? 'ring-4 ring-yellow-400' : ''
                            ]"
                            :style="`width: ${cellSize}px; height: ${cellSize}px;`"
                        >
                            <div :class="['w-[70%] h-[70%] rounded-sm', toVillageColorClass(getVillageAt(getCellX(index), getCellY(index)))]"></div>
                        </div>
                    </template>
                    <template v-else>
                        <div
                            class="border border-gray-300 flex items-center justify-center text-xs"
                            :style="`width: ${cellSize}px; height: ${cellSize}px;`"
                        ></div>
                    </template>
                </div>
